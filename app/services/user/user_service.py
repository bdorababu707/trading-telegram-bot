...
class UserService:
    ...
    async def create_telegram_user(
        ...
        try:
            ...
            logger.error("Error creating telegram user %s: %s", telegram_id, e)
            return None
    ...
    async def ensure_user_approved(message) -> bool:
        try:
            ...
            logger.error("Error ensuring user approved: %s", e)
            await message.answer("An error occurred while verifying your account. Please try again later.")
            return False

    @staticmethod
    async def get_user_by_phone_number(phone_number: str) -> Dict[str, Any]:
        """Fetch user by phone number"""
        try:
            logger.debug("Fetching user by phone number: %s", phone_number)
            user = await MongoHelper.find_one(collection=settings.DB_TABLE.USERS, query={"phone_number": phone_number})
            logger.debug("Fetched user: %s", user)
            return user
        except Exception as e:
            logger.error("Error fetching user by phone number %s: %s", phone_number, e)
            return None